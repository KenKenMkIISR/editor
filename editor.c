...
//各種配列用確保メモリ
unsigned char *EDITORRAM;

//配列EDITORRAM[]内にメモリ動的確保するためのポインタ
unsigned char *editormallocp;
...
unsigned char *temppath; //実体は配列RAM[]の中に確保する
...
//確保できない場合は、エラー表示し動作停止
	unsigned char *p;
	if(editormallocp+size>EDITORRAM+RAMSIZE){
...
// 画面縦横の切り替え
	set_lcdalign(!LCD_ALIGNMENT); //縦横切り替え
...
int compile_file(unsigned char *);
void pre_run(void);
void run_code(void);
void post_run(void);
...
// test 0:コンパイルと実行、0以外:コンパイルのみで終了
	...
	set_lcdalign(HORIZONTAL); //デフォルトは横方向で実行
